...
echo -e "${BLUE}🦉 BinaryPilot Build Script${NC}"
echo "================================"
...
uv run pyinstaller binarypilot.spec --noconfirm

RELEASE_DIR="dist/release"
mkdir -p "$RELEASE_DIR"

BINARY_NAME="binarypilot-${VERSION}-${OS_NAME}-${ARCH_NAME}"

if [ "$OS_NAME" = "windows" ]; then
    if [ ! -f "dist/binarypilot.exe" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    BINARY_NAME="${BINARY_NAME}.exe"
    cp "dist/binarypilot.exe" "$RELEASE_DIR/$BINARY_NAME"
    ...
    if [ ! -f "dist/binarypilot" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    cp "dist/binarypilot" "$RELEASE_DIR/$BINARY_NAME"
