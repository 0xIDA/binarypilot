...
from binarypilot.core import execution
...
from binarypilot.core.sessions import seed_initial_input
from binarypilot.tools.agents_graph.tools import agent_finish, stop_agent
...
async def test_reserve_stop_notifies_root_once(monkeypatch: pytest.MonkeyPatch) -> None:
    coordinator = AgentCoordinator()
    await coordinator.register("root", "binarypilot", parent_id=None)
...
async def test_budget_stop_unblocks_parked_agent() -> None:
    ...
    coordinator = AgentCoordinator()
    await coordinator.register("parent", "binarypilot", parent_id=None)
...
async def test_structured_provider_refusal_fails_interactive_agent(
    ...
    monkeypatch.setattr(
        "binarypilot.core.execution.Runner.run_streamed", lambda *_args, **_kwargs: stream
    )
    coordinator = AgentCoordinator()
    await coordinator.register("root", "binarypilot", parent_id=None)
