...
from binarypilot.config import codex, loader
from binarypilot.config.loader import load_settings
from binarypilot.config.models import BinaryPilotProvider, _NonStreamingModel
...
def _reset_settings(monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    for key in ("BINARYPILOT_LLM", "LLM_DISABLE_STREAMING"):
...
def test_get_model_wraps_when_disabled(
    ...
    inner = _DummyModel()
    monkeypatch.setattr("binarypilot.config.models.MultiProvider.get_model", lambda *_: inner)
    monkeypatch.setenv("LLM_DISABLE_STREAMING", "true")
    load_settings()

    model = BinaryPilotProvider().get_model("openai/gpt-4o-mini")
...
def test_get_model_does_not_wrap_subscription_model(
    ...
    model = BinaryPilotProvider().get_model("gpt-5.5")
