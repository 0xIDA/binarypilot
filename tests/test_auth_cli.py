"""Tests for the `binarypilot auth` CLI: subcommand routing and provider naming."""
...
from binarypilot.interface import auth_cli
...
def _tmp_store(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(codex, "AUTH_PATH", tmp_path / "home" / ".binarypilot" / "subscription-auth.json")
