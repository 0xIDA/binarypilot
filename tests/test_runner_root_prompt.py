"""Tests for root scan prompt options in run_binarypilot_scan.

Verify that ``root_instructions_override`` and ``extra_system_prompt_context``
flow through to the root agent's ``build_binarypilot_agent`` call.
"""
...
import binarypilot.tools.notes.tools as notes_tools
import binarypilot.tools.todo.tools as todo_tools
from binarypilot.core import runner
...
def _patch_engine_scaffold(
    ...
    """Stub out everything around build_binarypilot_agent and stop at run_agent_loop.

    Returns a dict that will be populated with the kwargs the runner passed to
    ``build_binarypilot_agent`` for the root agent.
    """
    ...
    def _build_binarypilot_agent(**kwargs: Any) -> object:
        if kwargs.get("is_root") and "kwargs" not in captured:
            captured["kwargs"] = kwargs
        return object()

    monkeypatch.setattr(runner, "build_binarypilot_agent", _build_binarypilot_agent)
...
async def test_root_prompt_options_flow_into_root_agent(
    ...
    await runner.run_binarypilot_scan(
