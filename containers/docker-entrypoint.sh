...
if [ -n "${BINARYPILOT_HOST_UID:-}" ] && [ "${BINARYPILOT_HOST_UID}" != "0" ] && [ "${BINARYPILOT_HOST_UID}" != "$(id -u)" ]; then
  exec sudo -E -- bash -c '
    set -e
    gid="${BINARYPILOT_HOST_GID:-$BINARYPILOT_HOST_UID}"
    old_uid="$1"
    old_gid="$2"
    export PATH="$3"
    shift 3
    sed -i "s|^pentester:x:${old_uid}:${old_gid}:|pentester:x:${BINARYPILOT_HOST_UID}:${gid}:|" /etc/passwd
    sed -i "s|^pentester:x:${old_gid}:|pentester:x:${gid}:|" /etc/group
    chown -R "${BINARYPILOT_HOST_UID}:${gid}" /home/pentester /app/certs
    chown "${BINARYPILOT_HOST_UID}:${gid}" /workspace
    exec setpriv --reuid "${BINARYPILOT_HOST_UID}" --regid "${gid}" --init-groups "$0" "$@"
  ' "$0" "$(id -u)" "$(id -g)" "$PATH" "$@"
fi
...
# hostname (rather than an IP literal), set BINARYPILOT_CAIDO_ALLOWED_DOMAINS to a
...
if [ -n "${BINARYPILOT_CAIDO_ALLOWED_DOMAINS:-}" ]; then
  IFS=',' read -ra _caido_domains <<< "${BINARYPILOT_CAIDO_ALLOWED_DOMAINS}"
