...
from binarypilot.config import codex
...
if TYPE_CHECKING:
    ...
    from binarypilot.config.settings import LlmSettings, ReasoningEffort, Settings
...
class _NonStreamingModel(Model):
    ...
    BinaryPilot uses only issues streamed requests, so such a gateway fails every
...
class BinaryPilotProvider(MultiProvider):
...
def configure_sdk_model_defaults(settings: Settings) -> None:
    """Apply BinaryPilot config to SDK-native defaults."""
    llm = settings.llm
...
def _configure_litellm_compatibility() -> None:
    ...
    # BinaryPilot uses LiteLLM's success callback to capture provider-reported cost.
...
def _install_openrouter_stream_cost_capture() -> None:
    ...
    lost and BinaryPilot falls back to a cost-map estimate that is missing entirely
    ...
    from binarypilot.report.state import streamed_openrouter_costs

    class _BinaryPilotOpenRouterStreamingHandler(OpenRouterChatCompletionStreamingHandler):
    ...
    class _BinaryPilotOpenrouterConfig(OpenrouterConfig):
        def get_model_response_iterator(
            self, streaming_response: Any, sync_stream: bool, json_mode: bool | None = False
        ) -> Any:
            return _BinaryPilotOpenRouterStreamingHandler(
    ...
    litellm.OpenrouterConfig = _BinaryPilotOpenrouterConfig  # type: ignore[misc]


_OPENROUTER_ATTRIBUTION_HEADERS = {
    "HTTP-Referer": "https://binarypilot.ai",
    "X-Title": "BinaryPilot",
...
def _register_litellm_cost_callback() -> None:
    ...
    from binarypilot.report.state import litellm_cost_callback
