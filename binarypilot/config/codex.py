...
AUTH_PATH = Path.home() / ".binarypilot" / "subscription-auth.json"
...
def _refresh_guard() -> Iterator[None]:
    """Serialize token refresh within (lock) and across (flock) BinaryPilot processes,
...
class CodexContentGuardrailError(Exception):
    ...
    def __init__(self, model: str, original: BaseException | None = None) -> None:
        ...
        super().__init__(
            ...
            f"Set BINARYPILOT_LLM to a model that isn't blocked and re-run."
        )
...
def get_valid_token() -> tuple[str, str]:
    ...
    record = read_record()
    if record is None:
        raise CodexAuthError("not_authenticated", "not signed in; run: binarypilot auth login")
...
def build_openai_client() -> AsyncOpenAI:
    ...
    return AsyncOpenAI(
        api_key="binarypilot-codex-oauth",  # placeholder; the hook overwrites Authorization
...
def subscription_model(model_name: str | None) -> str | None:
    """The model slug behind a ``chatgpt/<model>`` BINARYPILOT_LLM, or None."""
