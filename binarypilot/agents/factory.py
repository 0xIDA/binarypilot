"""Build SandboxAgents for root + child BinaryPilot runs."""
...
from binarypilot.tools.agents_graph.tools import (
    agent_finish,
    create_agent,
...
from binarypilot.tools.output_store import bound_and_store, bound_text
from binarypilot.tools.proxy.tools import (
    list_requests,
    list_sitemap,
    repeat_request,
    scope_rules,
...
from binarypilot.tools.reporting.tool import (
    create_dependency_report,
    create_vulnerability_report,
    get_report,
    list_reports,
)
from binarypilot.tools.respond.tool import respond_to_user
from binarypilot.tools.thinking.tool import think
from binarypilot.tools.todo.tools import (
    create_todo,
    delete_todo,
    list_todos,
    mark_todo_done,
...
from binarypilot.tools.web_search.tool import web_search
...
def _with_bounded_result(tool: FunctionTool) -> FunctionTool:
    ...
    if getattr(tool, "_binarypilot_bounded", False):
        return tool
    ...
    tool._binarypilot_bounded = True  # type: ignore[attr-defined]
    return tool
...
def _with_coerced_arguments(tool: FunctionTool) -> FunctionTool:
    if getattr(tool, "_binarypilot_coerced", False):
        return tool
    invoke_tool = tool.on_invoke_tool
    ...
    tool.on_invoke_tool = invoke
    tool._binarypilot_coerced = True  # type: ignore[attr-defined]
    return tool
...
# ``binarypilot.runtime.backends.register_backend``: register before the first
# ``build_binarypilot_agent`` call and every agent (root + children) gets them.
...
def _register_ctf_tools() -> None:
    # Lazy imports: factory import already pulls the whole SDK.
    from binarypilot.tools.flagyard import (  # noqa: PLC0415
...
def build_binarypilot_agent(
    *,
    name: str = "binarypilot",
...
def make_child_factory(
    *,
    ...
    def _factory(*, name: str, skills: list[str]) -> SandboxAgent[Any]:
        return build_binarypilot_agent(
            name=name,
            skills=skills,
            is_root=False,
            scan_mode=scan_mode,
