"""BinaryPilotDockerSandboxClient — preserves the image's ENTRYPOINT and adds
...
_SANDBOX_NETWORK_ENV = "BINARYPILOT_DOCKER_SANDBOX_NETWORK"
...
def _apply_resource_limits(create_kwargs: dict[str, Any]) -> None:
    ...
    mem_limit = os.environ.get("BINARYPILOT_SANDBOX_MEM_LIMIT", "").strip()
    if mem_limit:
        create_kwargs["mem_limit"] = mem_limit

    shm_size = os.environ.get("BINARYPILOT_SANDBOX_SHM_SIZE", "").strip()
    if shm_size:
        create_kwargs["shm_size"] = shm_size

    cpus = os.environ.get("BINARYPILOT_SANDBOX_CPUS", "").strip()
    if cpus:
    ...
    pids_limit = os.environ.get("BINARYPILOT_SANDBOX_PIDS_LIMIT", "").strip()
    if pids_limit:
...
def _apply_log_limits(create_kwargs: dict[str, Any]) -> None:
    ...
    ``max-size * max-file``. Set ``BINARYPILOT_SANDBOX_LOG_MAX_SIZE`` to ``0``/``off``
    to opt back out to docker's default."""
    max_size = os.environ.get("BINARYPILOT_SANDBOX_LOG_MAX_SIZE", "50m").strip()
    if max_size.lower() in ("0", "off", "none", "unlimited"):
        return
    max_file = os.environ.get("BINARYPILOT_SANDBOX_LOG_MAX_FILE", "3").strip() or "3"
...
def _apply_run_labels(create_kwargs: dict[str, Any]) -> None:
    ...
    if not isinstance(labels, dict):
        return
    labels["binarypilot-run-id"] = run_id
    run_type = os.getenv("BINARYPILOT_RUN_TYPE")
    if run_type:
        labels["binarypilot-run-type"] = run_type


class BinaryPilotDockerSandboxSession(DockerSandboxSession):
    sandbox_network: str = ""
...
class BinaryPilotDockerSandboxClient(DockerSandboxClient):
    ...
    binarypilot_bind_mounts: list[dict[str, Any]] | None = None
    ...
    async def _create_container(
        ...
        *,
        ...
        # BinaryPilot delta from the SDK body: drop ``entrypoint`` override and
        ...
        # BinaryPilot injections — append, don't overwrite, so FUSE/SYS_ADMIN survives.
        ...
        bind_mounts = self.binarypilot_bind_mounts or ()
        if bind_mounts:
            mounts = create_kwargs.setdefault("mounts", [])
    ...
    async def create(self, **kwargs: Any) -> SandboxSession:
        ...
        if network and isinstance(inner, DockerSandboxSession):
            inner.__class__ = BinaryPilotDockerSandboxSession
            cast("BinaryPilotDockerSandboxSession", inner).sandbox_network = network
        return session
