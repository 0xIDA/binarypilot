"""Sandbox backend registry — selected via BINARYPILOT_RUNTIME_BACKEND (default: docker)."""
...
async def _docker_backend(
    *,
    ...
    Uses :class:`BinaryPilotDockerSandboxClient` to inject NET_ADMIN /
    ...
    BinaryPilot manages session lifetime explicitly via ``client.delete()`` so we
    trigger ``start()`` ourselves.
    """
    import docker
    from agents.sandbox.sandboxes.docker import DockerSandboxClientOptions

    from binarypilot.runtime.docker_client import BinaryPilotDockerSandboxClient

    client = BinaryPilotDockerSandboxClient(docker.from_env())
    client.binarypilot_bind_mounts = bind_mounts or []
...
def get_backend(name: str) -> SandboxBackend:
    ...
    if backend is None:
        ...
        raise ValueError(
            f"Unknown BINARYPILOT_RUNTIME_BACKEND: {name!r} (supported: {supported})",
        )
