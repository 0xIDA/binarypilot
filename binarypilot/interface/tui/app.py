...
from binarypilot.config.models import is_recommended_or_frontier_model
from binarypilot.config.settings import DEFAULT_MAX_TURNS
from binarypilot.core.hooks import BudgetExceededError
from binarypilot.core.runner import run_binarypilot_scan
from binarypilot.interface.tui.live_view import TuiLiveView
from binarypilot.interface.tui.messages import send_user_message_to_agent
from binarypilot.interface.tui.renderers import render_tool_widget
from binarypilot.interface.tui.renderers.agent_message_renderer import AgentMessageRenderer
from binarypilot.interface.tui.renderers.user_message_renderer import UserMessageRenderer
from binarypilot.interface.utils import build_tui_stats_text
from binarypilot.report.state import ReportState, set_global_report_state
from binarypilot.report.writer import (
    guess_language_name,
    parse_fenced_code,
    resolve_lexer,
    safe_fence,
)
from binarypilot.runtime import session_manager
...
def get_package_version() -> str:
    try:
        return pkg_version("binarypilot-agent")
...
class ChatTextArea(TextArea):  # type: ignore[misc]
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._app_reference: BinaryPilotTUIApp | None = None

    def set_app_reference(self, app: "BinaryPilotTUIApp") -> None:
...
class HelpScreen(ModalScreen):  # type: ignore[misc]
    def compose(self) -> ComposeResult:
        yield Grid(
            Label("BinaryPilot Help", id="help_title"),
            Label(
                "F1        Help\nCtrl+Q/C  Quit\nESC       Stop Agent\n"
...
class QuitScreen(ModalScreen):  # type: ignore[misc]
    def compose(self) -> ComposeResult:
        yield Grid(
            Label("Quit BinaryPilot?", id="quit_title"),
            Grid(
...
class BinaryPilotTUIApp(App):  # type: ignore[misc]
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        from binarypilot.core.agents import AgentCoordinator
    ...
    def _merge_renderables(renderables: list[Any]) -> Text:
        ...
        for i, item in enumerate(renderables):
            if i > 0:
                combined.append("\n")
            BinaryPilotTUIApp._append_renderable(combined, item)
        return BinaryPilotTUIApp._sanitize_text(combined)
    ...
    def _append_renderable(combined: Text, item: Any) -> None:
        ...
        if isinstance(item, Text):
            combined.append_text(BinaryPilotTUIApp._sanitize_text(item))
        elif isinstance(item, Group):
            for j, sub in enumerate(item.renderables):
                if j > 0:
                    combined.append("\n")
                BinaryPilotTUIApp._append_renderable(combined, sub)
        else:
            inner = getattr(item, "content", None) or getattr(item, "renderable", None)
            if inner is not None:
                BinaryPilotTUIApp._append_renderable(combined, inner)
            else:
            ...
            try:
                ...
                try:
                    if not self._scan_stop_event.is_set():
                        image = load_settings().runtime.image or "binarypilot-sandbox:latest"
                        loop.run_until_complete(
                            run_binarypilot_scan(
    ...
    def action_open_viewer(self) -> None:
        if self._viewer_url:
            ...
            return
        try:
            from binarypilot.interface.viewer.server import authorized_url, bundle_is_built, serve
    ...
    def _clean_copied_text(text: str) -> str:
        ...
        for line in lines:
            ...
            if stripped in BinaryPilotTUIApp._DECORATIVE_LINES:
                continue
            if stripped and all(c == "─" for c in stripped):
                continue
            out = line
            for prefix in BinaryPilotTUIApp._ICON_PREFIXES:
