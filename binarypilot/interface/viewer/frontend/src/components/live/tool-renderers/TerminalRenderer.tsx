...
// TUI strips them in binarypilot/interface/tui/renderers/shell_renderer.py). Only a
...
function cleanOutput(raw: string, command: string = ""): string {
  ...
  if (cleaned.trim()) {
    ...
    for (const line of lines) {
      ...
      // Skip [BINARYPILOT_N]$ prompt lines
      if (/^\[BINARYPILOT_\d+\]\$\s*/.test(line)) continue;
    ...
    // Strip trailing [BINARYPILOT_N]$ lines
    while (filtered.length > 0 && /^\[BINARYPILOT_\d+\]\$\s*/.test(filtered[filtered.length - 1])) {
      filtered.pop();
    }
