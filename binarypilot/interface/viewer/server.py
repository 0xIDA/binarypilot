...
# port is appended (``binarypilot_viewer_session_<port>``) because browsers scope
...
SESSION_COOKIE_PREFIX = "binarypilot_viewer_session"
...
class _ViewerState:
    def __init__(
        ...
        # The binarypilot_runs directory that holds the launched run; used to
        ...
        # standalone ``binarypilot view`` / finished runs, so steering is unavailable.
...
def _make_handler(state: _ViewerState) -> type[BaseHTTPRequestHandler]:
    class ViewerHandler(BaseHTTPRequestHandler):
        server_version = "BinaryPilotViewer/1.0"
...
def serve(
    ...
    *,
    ...
    use, falls back to an ephemeral port. Reused by both the ``binarypilot view``
    ...
    ``None`` (standalone ``binarypilot view``), steering is reported unavailable.
    """
    ...
    thread = threading.Thread(target=httpd.serve_forever, name="binarypilot-viewer", daemon=True)
    thread.start()
