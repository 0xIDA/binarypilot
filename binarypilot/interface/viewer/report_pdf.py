...
The layout mirrors the BinaryPilot cloud pentest report (cover page, executive
...
the user's own hands. BinaryPilot cannot read the delivered report.
"""
...
class _LogoMark(Flowable):  # type: ignore[misc]  # reportlab base is untyped
    """The rounded-square BinaryPilot mark drawn inline (no raster asset to ship)."""
...
def _cover(
    ...
    header = Table(
        [[_LogoMark(30), Paragraph("BinaryPilot", styles["wordmark"])]],
...
def generate_report_pdf(run_dir: Path) -> bytes:
    ...
    doc = SimpleDocTemplate(
        buffer,
        pagesize=A4,
        title="BinaryPilot Security Report",
        author="BinaryPilot",
        leftMargin=20 * mm,
        rightMargin=20 * mm,
        topMargin=22 * mm,
