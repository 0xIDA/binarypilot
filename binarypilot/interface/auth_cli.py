"""`binarypilot auth` — ChatGPT subscription sign-in (login / status / logout).

Signing in only stores OAuth tokens (``~/.binarypilot/subscription-auth.json``); model
selection stays with ``BINARYPILOT_LLM``. A ``chatgpt/<model>`` BINARYPILOT_LLM runs on the
subscription.
"""
...
from binarypilot.config import codex, load_settings
...
_USAGE = "Usage:\n  binarypilot auth login chatgpt [--manual]\n  binarypilot auth status\n  binarypilot auth logout"


def run_auth(argv: list[str]) -> int:
    """Entry point for ``binarypilot auth …``. Returns a process exit code."""
    console = Console()
    # Bare `binarypilot auth` (no subcommand) defaults to login.
...
def _login(console: Console, argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="binarypilot auth login", add_help=True)
    parser.add_argument(
        "provider",
        nargs="?",
...
def _status(console: Console) -> int:
    ...
    if record is None:
        console.print("[yellow]Not signed in.[/] Run [cyan]binarypilot auth login chatgpt[/] to sign in.")
        return 1
    ...
    if codex.subscription_model(settings.llm.model):
        console.print(f"  Runs use the subscription (BINARYPILOT_LLM=[bold]{settings.llm.model}[/]).")
    else:
        console.print(
            "  [yellow]Note:[/] set [cyan]BINARYPILOT_LLM[/] to e.g. [cyan]chatgpt/gpt-5.4[/] "
...
def _print_success(console: Console) -> None:
    ...
    text.append("BINARYPILOT_LLM", style="bold white")
    ...
    text.append("binarypilot --target https://example.com", style="bold cyan")
    console.print()
    console.print(
        Panel(
            text,
...
def _logo_img_tag() -> str:
    """Return an ``<img>`` for the BinaryPilot logo as an inline data URI, or "".
...
<title>BinaryPilot — signed in</title>
<style>
...
<body>
  ...
  <div class="brand">BinaryPilot</div>
  <h1>You're signed in</h1>
  <main class="card">
    <div class="badge">✓</div>
    <p class="msg">BinaryPilot is connected to your ChatGPT subscription. Head back to your
