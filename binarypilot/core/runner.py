"""Top-level BinaryPilot scan runner."""
...
from binarypilot.agents.factory import build_binarypilot_agent, make_child_factory
from binarypilot.agents.prompt import render_system_prompt
...
from binarypilot.core.execution import (
    respawn_subagents,
    run_agent_loop,
)
from binarypilot.core.execution import (
...
from binarypilot.core.hooks import BudgetExceededError, ReportUsageHooks, recomputed_budget_flags
from binarypilot.core.inputs import (
    build_root_task,
    build_scope_context,
    make_model_settings,
)
from binarypilot.core.paths import run_dir_for, runtime_state_dir
from binarypilot.core.sessions import open_agent_session
from binarypilot.report.state import get_global_report_state
from binarypilot.runtime import session_manager
from binarypilot.telemetry.logging import set_scan_id, setup_scan_logging
from binarypilot.tools.output_store import (
    WORKSPACE_SPILL_DIR,
...
if TYPE_CHECKING:
    ...
    from binarypilot.runtime.status import StatusSink
...
async def run_binarypilot_scan(
    *,
    ...
    """Run or resume one BinaryPilot scan against a sandbox.
    ...
    logger.info(
        "%s BinaryPilot scan %s (image=%s, max_turns=%d, interactive=%s, run_dir=%s)",
    ...
    if not resolved_model:
        raise RuntimeError(
            "No LLM model configured. Set BINARYPILOT_LLM env or pass model= to run_binarypilot_scan().",
        )
    ...
    from binarypilot.tools.notes.tools import hydrate_notes_from_disk
    from binarypilot.tools.todo.tools import hydrate_todos_from_disk
    ...
    try:
        ...
        run_config = RunConfig(
            ...
            model_provider=BinaryPilotProvider(),
        ...
        root_agent = build_binarypilot_agent(
            name="BinaryPilot",
            skills=skills,
            is_root=True,
            scan_mode=scan_mode,
        ...
        logger.warning(
            ...
            "Resume with 'binarypilot --resume %s' once the limit clears.",
            scan_id,
            exc,
            scan_id,
        )
        ...
        logger.exception("BinaryPilot scan %s failed", scan_id)
        if root_id is not None:
            await coordinator.cancel_descendants(root_id)
            with contextlib.suppress(Exception):
        ...
        logger.info("BinaryPilot scan %s done", scan_id)
        teardown_logging()
