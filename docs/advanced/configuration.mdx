---
title: "Configuration"
description: "Environment variables for BinaryPilot"
---

Configure BinaryPilot using environment variables or a config file.

## LLM Configuration

<ParamField path="BINARYPILOT_LLM" type="string" required>
...
<ParamField path="BINARYPILOT_LLM_MAX_RETRIES" default="5" type="integer">
  Maximum number of retries for LLM API calls on transient failures.
</ParamField>

<ParamField path="BINARYPILOT_REASONING_EFFORT" default="high" type="string">
...
<ParamField path="BINARYPILOT_MEMORY_COMPRESSOR_TIMEOUT" default="30" type="integer">
...
<ParamField path="BINARYPILOT_DEDUPE_MODEL" type="string">
  Model used to judge whether a candidate finding duplicates an existing report.
  Falls back to `BINARYPILOT_LLM` when unset.
</ParamField>
...
<ParamField path="BINARYPILOT_DEDUPE_REASONING_EFFORT" type="string">
...
<ParamField path="BINARYPILOT_TELEMETRY" default="1" type="string">
...
<ParamField path="TRACELOOP_BASE_URL" type="string">
  OTLP/Traceloop base URL for remote OpenTelemetry export. If unset, BinaryPilot keeps traces local only.
</ParamField>
...
When remote OTEL vars are not set, BinaryPilot still writes complete run telemetry locally to:

```bash
binarypilot_runs/<run_name>/events.jsonl
```

When remote vars are set, BinaryPilot dual-writes telemetry to both local JSONL and the remote OTEL endpoint.

## Docker Configuration

<ParamField path="BINARYPILOT_IMAGE" default="ghcr.io/usebinarypilot/binarypilot-sandbox:1.2.0" type="string">
...
<ParamField path="BINARYPILOT_RUNTIME_BACKEND" default="docker" type="string">
  Runtime backend for the sandbox environment.
</ParamField>

## Sandbox Configuration

<ParamField path="BINARYPILOT_SANDBOX_EXECUTION_TIMEOUT" default="120" type="integer">
  Maximum execution time in seconds for sandbox operations.
</ParamField>

<ParamField path="BINARYPILOT_SANDBOX_CONNECT_TIMEOUT" default="10" type="integer">
  Timeout in seconds for connecting to the sandbox container.
</ParamField>

## Config File

BinaryPilot stores configuration in `~/.binarypilot/cli-config.json`. You can also specify a custom config file:

```bash
binarypilot --target ./app --config /path/to/config.json
```
...
{
  "env": {
    "BINARYPILOT_LLM": "openai/gpt-5.4",
    "LLM_API_KEY": "sk-...",
    "BINARYPILOT_REASONING_EFFORT": "high"
  }
}
```
...
export BINARYPILOT_SANDBOX_EXECUTION_TIMEOUT="300"
