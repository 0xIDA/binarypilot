---
title: "CLI Reference"
description: "Command-line options for BinaryPilot"
---

## Basic Usage

```bash
binarypilot (--target <target> | --target-list <path>) [options]
```
...
<ParamField path="--config" type="string">
  Path to a custom config file (JSON) to use instead of `~/.binarypilot/cli-config.json`.
</ParamField>
...
<ParamField path="--max-budget" type="number">
  ...
  **Limitations**
  ...
  - For LiteLLM-routed models, BinaryPilot enables streaming success callbacks to
...
binarypilot --target https://app.com --instruction "Use credentials: user:pass"

# Focused testing
binarypilot --target api.example.com --instruction "Focus on IDOR and auth bypass"

# CI/CD mode
binarypilot -n --target ./ --scan-mode quick

# Cap cost and per-agent turns
binarypilot --target https://example.com --max-budget 25 --max-turns 300
...
binarypilot -t https://github.com/org/app -t https://staging.example.com
