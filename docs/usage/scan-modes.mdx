...
BinaryPilot offers three scan modes to balance speed and thoroughness.

## Quick

```bash
binarypilot --target ./app --scan-mode quick
```
...
```bash
binarypilot --target ./app --scan-mode standard
```
...
```bash
binarypilot --target ./app --scan-mode deep
```
