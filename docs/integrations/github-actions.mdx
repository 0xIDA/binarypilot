...
description: "Run BinaryPilot security scans on every pull request"
---

Integrate BinaryPilot into your GitHub workflow to catch vulnerabilities before they reach production.
...
| Secret | Description |
|--------|-------------|
| `BINARYPILOT_LLM` | Model name (e.g., `openai/gpt-5.4`) |
...
<Note>
For pull_request workflows, BinaryPilot automatically uses changed-files diff-scope in CI/headless runs. If diff resolution fails, ensure full history is fetched (`fetch-depth: 0`) or set `--diff-base`.
</Note>
